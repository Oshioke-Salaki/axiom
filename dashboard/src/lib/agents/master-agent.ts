...
import type { SentimentAnalysis } from "./sentiment-agent";
import type { OnchainAnalysis } from "./onchain-agent";
...
class MasterAgent extends AxiomAgent {
  ...
  async runResearchPipeline(
    ...
    const { SentimentAgent } = await import("./sentiment-agent");
    const { OnchainDataAgent } = await import("./onchain-agent");
